...
srun --ntasks 1 --mem-per-cpu 1G --nodes 1 $SINGULARITY_PREFIX $BINARY --run_mode=learner &

for ((id=0; id<$NUM_ACTORS; id++)); do
  sleep 3 # To avoid racing condition in singularity
  echo "Running an actor $id"
  srun --ntasks 1 --mem-per-cpu 1G --nodes 1 $SINGULARITY_PREFIX $BINARY --run_mode=actor --task=$id &
done
